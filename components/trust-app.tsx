  ...
  return (
    <div className="app">
      <aside className="side" aria-label="Workspace">
        <a className="mark" href="#" aria-label="Trust Library">
          T
        </a>
      ...
      <main className="stage">
        ...
        <section className="role-context is-quiet" aria-label="Current workflow">
          <div>
          ...
          <section className="library-shell">
            <section className="browse">
              <div className="collection-top">
                <div>
                  <span>Library</span>
                  <h1>{role.collectionTitle}</h1>
                </div>
                <p>{role.collectionMeta}</p>
              </div>
  ...
  return (
    <main className="role-gate">
      <section className="gate-intro">
        ...
        <p>Each role opens into a quieter view built around the work that person needs to do.</p>
      </section>

      <section className="role-grid" aria-label="Choose user type">
  ...
  return (
    <aside className="focus-panel" aria-label="Selected video">
      <div className="panel-top">
        <span>Selected</span>
        <button className="icon-mini" aria-label="More">
          <MoreHorizontal />
        </button>
      </div>
    ...
    <>
      <div className="workflow-shell">
        <section className="prospect-brief">
          <div className="collection-top">
            <div>
              <span>Prospect Brief</span>
              <h1>Kitchen remodel lead</h1>
            </div>
            <p>New inquiry</p>
          </div>
          <div className="brief-grid">
            <label>
              <span>Service</span>
              <input defaultValue="Kitchen remodel" />
            </label>
            <label>
              <span>Stage</span>
              <input defaultValue="Before estimate call" />
            </label>
            <label className="wide-field">
              <span>Notes</span>
              <textarea defaultValue="Concerned about price range, timeline disruption, and whether the design process is clear." />
            </label>
            <label>
              <span>Objection</span>
              <input defaultValue="Budget anxiety" />
            </label>
            <label>
              <span>Goal</span>
              <input defaultValue="Create confidence before call" />
            </label>
          </div>
          <button className="wide-action">
            <Sparkles />
            Recommend trust journey
          </button>
        </section>
          ...
          <div className="workflow-steps">
            {["Capture context", "Review recommendation", "Share private link", "Watch activity"].map((step, index) => (
              <article className={index === 0 ? "workflow-step is-done" : index === 1 ? "workflow-step is-active" : "workflow-step"} key={step}>
                ...
                <div>
                  <strong>{step}</strong>
                  <p>{index === 1 ? "Six videos selected for pricing and process confidence." : "Keep the rep moving without CRM overhead."}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="share-preview">
        <div className="cover-strip">
          {videos.slice(0, 3).map((video) => (
            <div className="cover-tile" key={video.id} style={{ backgroundImage: `url(${video.image})` }} />
          ))}
        </div>
...
function SourcesView() {
  ...
  return (
    <section className="sources-layout">
      <header>
        <span>Sources</span>
        <h1>Connect only the channels this business owns.</h1>
      </header>
      <div className="source-stack">
        {sources.map(([title, description, action]) => (
          <article className={action === "Connected" ? "source-row is-connected" : "source-row"} key={title}>
            <Plug />
            <div>
              <h2>{title}</h2>
              <p>{description}</p>
