...
The public viewer has separate standalone and inline-embed presentations. Drive uses Google's default preview player, loaded immediately for the selected asset only, without a second TrustTale play overlay. Drive playback does not use the Vercel byte-range route. Google controls quality, buffering, volume and its overlays; iPhone volume can be controlled by hardware buttons. Other asset types retain explicit activation. Direct-file native playback has controls and a volume slider below the picture. The journey description is visible beneath its main heading, while asset-specific notes remain with the asset.
