...
export function buildReport({
  ...
  uploadedSpendRows = [],
  windsorRows = [],
  attributionRows = windsorRows,
  jobtreadRows = [],
  rules,
  allowArchivedSpend = true,
}) {
  const csvSpendRows = normalizeSpendRows(rowsToObjects(parseCsv(spendCsv)), {
    startDate,
    endDate,
    timeZone: rules?.timezone,
  });
  const workbookSpendRows = normalizeSpendRows(uploadedSpendRows, {
    startDate,
    endDate,
    timeZone: rules?.timezone,
  });
  ...
  const archivedSpend =
    providedSpendRows.length || !allowArchivedSpend
      ? { rows: [], sources: [], warnings: [] }
      : loadArchivedSpendRows({ startDate, endDate });
  const spendRows = providedSpendRows.length
    ? providedSpendRows
    : normalizeSpendRows(archivedSpend.rows, { startDate, endDate, timeZone: rules?.timezone });
  const leads = normalizeWindsorContacts(windsorRows, { startDate, endDate, rules });
  const attributionLeads = normalizeWindsorContacts(attributionRows, { rules });
  const jobs = normalizeJobTreadJobs(jobtreadRows, { startDate, endDate, rules });
  ...
  const metrics = calculateReportMetrics({
    ...
    attributionMatches: attributionMatches.matched,
    rules,
  });
  const spendSource = providedSpendRows.length ? "uploaded" : spendRows.length && allowArchivedSpend ? "archive" : "none";
  ...
  return {
    ...
    detail: {
      ...
      jobs,
      attributionLeads,
      salesDocuments: metrics.totalSalesReport.rows,
      attributedJobs: jobs.map(job => {
        const lead = attributionMatches.matched.find(match => match.job.id === job.id)?.lead;
        return { ...job, lead, source: lead?.source || job.source, campaign: lead?.campaign || job.campaign };
      }),
      spendRows,
