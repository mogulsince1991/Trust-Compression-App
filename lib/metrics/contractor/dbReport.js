...
export function buildReportFromDatabaseRows({ client, startDate, endDate, leads = [], jobs = [], spendRows = [], rules }) {
  return buildReport({
    client,
    startDate,
    endDate,
    ...
    jobtreadRows: jobs.map((row) => ({
      approvedSalesDocuments: row.raw?.approvedSalesDocuments ?? [],
      closedOn: row.raw?.closedOn,
      primaryContact: row.raw?.primaryContact,
      id: row.external_id || row.id,
...
export function reportBreakdowns(metrics) {
  return {
    spend: metrics.spend,
    totalSalesReport: metrics.totalSalesReport,
