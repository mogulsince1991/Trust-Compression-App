...
export function easternDateTime(value, rules = METRIC_RULE_DEFINITIONS) {
  const parsed = parseDateValue(value);
  if (!parsed) return "";
  return formatterFor(rules.timezone ?? METRIC_RULE_DEFINITIONS.timezone).format(parsed.date);
}
