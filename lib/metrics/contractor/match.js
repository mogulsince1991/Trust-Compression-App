export function matchRecords(leads, jobs) {
  ...
  for (const job of jobs) {
    const jobNameKeys = [...normalizeNameKeys(job.customer), ...normalizeNameKeys(job.primaryContact?.name)];
    ...
    const lead = (job.phone && leadByPhone.get(job.phone)) || (job.email && leadByEmail.get(job.email)) || (nameMatches.length === 1 ? nameMatches[0] : null);
    if (!lead) continue;
    matched.push({ lead, job, matchKey: job.phone && job.phone === lead.phone ? "phone" : job.email && job.email === lead.email ? "email" : "name" });
