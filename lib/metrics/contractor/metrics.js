...
export function calculateReportMetrics({ spendRows, leads, jobs, matches, attributionMatches = matches }) {
  const appointmentJobs = jobs.filter((job) => job.inReportAppointment !== false);
  const soldJobs = jobs.filter((job) => isSoldJob(job) && job.inReportSold !== false);
  ...
  const paidLeads = leads.filter((lead) => sourceBucket(lead) === "paid");
  const organicLeads = leads.filter((lead) => sourceBucket(lead) !== "paid");
  const paidAppointmentJobs = attributedAppointmentJobs.filter((job) => sourceBucket(job) === "paid");
  const organicAppointmentJobs = attributedAppointmentJobs.filter((job) => sourceBucket(job) !== "paid");
  const paidSoldJobs = attributedSoldJobs.filter((job) => sourceBucket(job) === "paid");
  const organicSoldJobs = attributedSoldJobs.filter((job) => sourceBucket(job) !== "paid");
  ...
  const byVendor = [...groupBy(spendRows, (row) => row.vendor).entries()].map(([vendor, rows]) => {
    ...
    const vendorLeads = leads.filter((lead) => matchesVendor(vendor, lead));
    const vendorJobs = attributedAppointmentJobs.filter((job) => matchesVendor(vendor, job));
    const vendorSold = attributedSoldJobs.filter((job) => matchesVendor(vendor, job));
  ...
  const byCampaign = [...groupBy(spendRows, (row) => row.campaign || row.vendor).entries()].map(([campaign, rows]) => {
    ...
    const campaignSold = campaignJobs.filter(isSoldJob);
  ...
  return {
    ...
    byDesignConsultant,
    byLeadSource,
    closingOutcomes: analyzeClosingOutcomes(appointmentJobs),
    jobsSoldDetail: soldJobs.map((job) => ({
      ...(() => {
        const attributed = attributionForJob(job);
        return {
          attributedSource: attributed.source,
          sourceBucket: sourceBucket(attributed),
          leadCreatedDate: attributed.lead?.createdDate ?? "",
          leadCreatedEastern: easternDateTime(attributed.lead?.createdDate),
