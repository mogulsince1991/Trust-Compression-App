...
import { summarizeSalesRows } from "./salesDocuments.js";
...
export function calculateReportMetrics({ spendRows, leads, jobs, matches, attributionMatches = matches, rules }) {
  ...
  const totalSalesReport = summarizeSalesRows(jobs.flatMap(job => {
    const attributed = attributionForJob(job);
    return job.salesRows.map(row => ({ ...row, source: attributed.source, campaign: attributed.campaign,
      setter: attributed.lead?.estimator ?? "", sourceBucket: sourceBucket(attributed, rules) }));
  }));
  ...
  const vendorNames = new Set([...spendRows.map(row => canonicalPaidVendor(row.vendor, rules)), ...paidSoldJobs.map(job => paidVendorFor(job, rules))]);
  const byVendor = [...vendorNames].map(vendor => {
    const rows = spendRows.filter(row => canonicalPaidVendor(row.vendor, rules) === vendor);
    ...
    const vendorSold = paidSoldJobs.filter(job => paidVendorFor(job, rules) === vendor);
  ...
  const byCampaign = [...groupBy(spendRows, (row) => row.campaign || row.vendor).entries()].map(([campaign, rows]) => {
    ...
    const campaignSold = attributedSoldJobs.filter(job => rows.some(row => row.campaign && row.campaign === job.campaign));
