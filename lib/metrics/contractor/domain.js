...
export function isSoldJob(job, rules = METRIC_RULE_DEFINITIONS) {
  ...
  return getSoldStatuses(rules).has(String(job.status ?? "").trim().toLowerCase());
}
...
function getSoldStatuses(rules = METRIC_RULE_DEFINITIONS) {
  return new Set(
    (rules.classification?.soldJob?.statuses ?? METRIC_RULE_DEFINITIONS.classification.soldJob.statuses).map((value) =>
