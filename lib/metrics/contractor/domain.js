...
export function hasNotLeadTag(tags, rules = METRIC_RULE_DEFINITIONS) {
  const text = Array.isArray(tags) ? tags.join(" ") : String(tags ?? "");
  const normalized = text.toLowerCase();
  return getExcludedLeadTagPhrases(rules).some((phrase) => normalized.includes(phrase));
}

export function isSoldJob(job, rules = METRIC_RULE_DEFINITIONS) {
  return Boolean(job.salesRows?.some(row => !row.cancelled));
}
