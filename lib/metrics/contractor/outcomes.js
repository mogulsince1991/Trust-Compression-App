...
const OUTCOME_RULES = METRIC_RULE_DEFINITIONS.closingOutcomeRules.map((rule) => ({
  ...rule,
  pattern: compilePattern(rule.pattern),
}));

export function analyzeClosingOutcomes(jobs) {
  ...
  for (const job of unsold) {
    ...
    const rule = OUTCOME_RULES.find((candidate) => candidate.pattern.test(text));
