...
export function canonicalPaidVendor(value, rules = METRIC_RULE_DEFINITIONS) {
  const key = normalizeKey(value);
  return getPaidVendorAliases(rules).find(rule => normalizeKey(rule.vendor) === key || rule.aliases.some(alias => normalizeKey(alias) === key))?.vendor || value;
}

export function paidVendorFor(record, rules = METRIC_RULE_DEFINITIONS) {
  if (sourceBucket(record, rules) !== "paid") return "";
  return getPaidVendorAliases(rules).find(rule => matchesVendor(rule.vendor, record, rules))?.vendor || record.source || "Other paid";
}
