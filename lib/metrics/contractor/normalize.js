...
import { salesRowsForJob } from "./salesDocuments.js";
...
export function normalizeWindsorContacts(rows, { startDate, endDate, rules } = {}) {
  ...
  return rows
    ...
    .map((row) => ({
      ...
      estimator: normalizeText(row.contact_estimator ?? row.estimator ?? row.setter),
...
export function normalizeJobTreadJobs(rows, { startDate, endDate, rules } = {}) {
  ...
  return rows
    .map((row) => {
      const baseJob = {
        id: normalizeText(row.jobId ?? row.id ?? row.jobNumber ?? row.number ?? row["Job #"]),
        ...
        closedOn: row.closedOn ?? row.closed_on ?? null,
        approvedSalesDocuments: row.approvedSalesDocuments ?? [],
      ...
      const configuredSoldDate = { value: baseJob.jobSoldDate ?? baseJob.soldDate, source: "informational-only" };
      const salesRows = salesRowsForJob({ ...baseJob, cancelled: matchesCancelled(baseJob, rules) }, startDate, endDate);
      const revenue = salesRows.filter(row => !row.cancelled).reduce((total, row) => total + row.amount, 0);
      return {
        ...baseJob,
        ...
        netSales: revenue,
        salesRows,
        resolvedRevenueField: "documents.priceWithTax",
    ...
    .map((job) => ({
      ...
      inReportSold: job.salesRows.some(row => !row.cancelled),
    }))
    .filter((job) => {
      if (!job.id) return false;
      if (!startDate) return true;
      return job.inReportAppointment || job.salesRows.length > 0;
    });
}
